...
// ── Draggable capsule (desktop interactive board) ────────────────
function FloatingIcon({ tech, delay, containerRef }) {
  return (
    <motion.div
      ...
      style={{ position: 'absolute', left: `${tech.x}%`, top: `${tech.y}%`, translateX: '-50%', translateY: '-50%', cursor: 'none', zIndex: 10, touchAction: 'none', userSelect: 'none' }}
    >
      <motion.div
        whileHover={{ boxShadow: `0 0 20px ${tech.color}, 0 0 40px ${tech.color}80` }}
        className="flex items-center gap-2 px-3 py-2 rounded-xl"
        style={{ background: 'rgba(10, 10, 30, 0.85)', border: `1px solid ${tech.color}60`, boxShadow: `0 0 8px ${tech.color}30`, backdropFilter: 'blur(8px)', minWidth: 80, whiteSpace: 'nowrap' }}
      >
        <tech.Icon />
        <span className="font-poppins font-semibold text-xs" style={{ color: tech.color, textShadow: `0 0 6px ${tech.color}` }}>
          {tech.name}
        </span>
      </motion.div>
    </motion.div>
    ...
    <div className="flex flex-wrap justify-center gap-3 py-6">
      {techIcons.map((tech, i) => (
        <motion.div
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-xl"
          style={{ background: 'rgba(10, 10, 30, 0.85)', border: `1px solid ${tech.color}60`, boxShadow: `0 0 8px ${tech.color}30`, backdropFilter: 'blur(8px)' }}
        >
          <tech.Icon />
          <span className="font-poppins font-semibold text-xs" style={{ color: tech.color, textShadow: `0 0 6px ${tech.color}` }}>
            {tech.name}
...
export default function StackSection() {
  ...
  return (
    <section id="stack" className="relative py-12 sm:py-20 overflow-x-hidden" style={{ zIndex: 1 }}>
      <div className="container mx-auto px-4 sm:px-8">
        ...
        >
          <h2
            ...
            style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', background: 'linear-gradient(135deg, #00FFFF, #9900FF, #FF007F)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', filter: 'drop-shadow(0 0 15px rgba(0,255,255,0.4))' }}
          >
            Habib's Stack
          </h2>
          <p className="font-poppins text-gray-300 max-w-lg mx-auto text-sm">
        ...
        <div className="block md:hidden">
          <motion.div
            ...
            style={{ border: '1px solid rgba(0,255,255,0.25)', background: 'rgba(0,0,20,0.85)', boxShadow: '0 0 30px rgba(0,255,255,0.1)' }}
          >
            <StaticTechGrid />
          </motion.div>
        </div>

        {/* ── Desktop: draggable circuit board ── */}
        <div className="hidden md:block">
          ...
          >
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 700 380" preserveAspectRatio="none">
              ...
              <rect x="10" y="10" width="680" height="360" rx="20" ry="20" fill="none" stroke="url(#cyanPurple)" strokeWidth="2" filter="url(#glow)" opacity="0.8"/>
            ...
            <div className="absolute" style={{ inset: '12px', background: 'rgba(0,0,20,0.85)', borderRadius: '16px', backdropFilter: 'blur(4px)' }} />

            {/* Draggable icons */}
            {techIcons.map((tech, i) => (
              <FloatingIcon key={`${tech.name}-${i}`} tech={tech} delay={0.3 + i * 0.08} containerRef={containerRef} />
            ))}
          </motion.div>
        </div>
        ...
        >
          <motion.button
            ...
            style={{ background: 'linear-gradient(135deg, #0066FF, #00FFFF)', color: '#03030F', cursor: 'none', boxShadow: '0 0 15px rgba(0,255,255,0.35)' }}
          >
            Let's Get in Touch
          </motion.button>
          <motion.button
