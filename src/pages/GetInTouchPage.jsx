  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 200, height: 200 }}>
      ...
      >
        ...
        <img src={myPhoto} alt="Habib Headshot" style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
      </motion.div>
    </div>
  )
}
...
function CircuitFormFrame({ children }) {
  return (
    <div className="relative p-5">
      {['tl', 'tr', 'bl', 'br'].map(pos => (
        <svg key={pos} width="30" height="30"
          style={{ position: 'absolute', ...(pos.includes('t') ? { top: 0 } : { bottom: 0 }), ...(pos.includes('l') ? { left: 0 } : { right: 0 }), transform: `scale(${pos.includes('r') ? -1 : 1}, ${pos.includes('b') ? -1 : 1})` }}
          viewBox="0 0 30 30"
        >
          <path d="M3,3 L15,3 L15,5 L5,5 L5,15 L3,15 Z" fill="#00FFFF" opacity="0.6" />
        </svg>
      ))}
      <div style={{ border: '1px solid rgba(0,255,255,0.25)', borderRadius: 12, padding: '24px', background: 'rgba(5,5,20,0.6)', backdropFilter: 'blur(10px)', boxShadow: '0 0 30px rgba(0,255,255,0.1)' }}>
        {children}
      </div>
    </div>
  )
    ...
    >
      ...
      <div className="flex-1 flex flex-col md:flex-row items-center md:items-start px-4 sm:px-8 py-4 gap-6 md:gap-10">
        ...
        <motion.div
          ...
          className="flex-shrink-0 flex justify-center w-full md:w-auto md:pt-4"
        >
          <PulsingHeadshot />
        </motion.div>
          ...
          <CircuitFormFrame>
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="Your Full Name" className="cyber-input" required />
                <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="Your Email Address" className="cyber-input" required />
              </div>
              <div className="mb-4 relative">
                <textarea name="inquiry" value={form.inquiry} onChange={handleChange} placeholder="Describe Your Project or Inquiry" rows={4} className="cyber-input resize-none" />
                <div style={{ position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(0deg, rgba(0,255,255,0.02) 0px, transparent 1px, transparent 20px), repeating-linear-gradient(90deg, rgba(0,255,255,0.02) 0px, transparent 1px, transparent 20px)', borderRadius: 8, pointerEvents: 'none' }} />
              </div>
              <div className="flex justify-end">
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="font-poppins font-semibold text-sm px-6 sm:px-8 py-3 rounded-lg"
                  style={{ background: 'linear-gradient(135deg, #5500AA, #9900FF)', color: 'white', cursor: 'none', boxShadow: '0 0 15px rgba(150,0,255,0.4)', border: '1px solid rgba(150,0,255,0.5)' }}
                >
